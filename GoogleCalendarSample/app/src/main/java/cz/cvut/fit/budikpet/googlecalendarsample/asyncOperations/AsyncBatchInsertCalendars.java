...
public class AsyncBatchInsertCalendars extends CalendarAsyncTask {
	...
	protected void doInBackground() throws IOException {
		...
		for (Calendar calendar : calendars) {
			client.calendars().insert(calendar).setFields(cz.cvut.fit.budikpet.googlecalendarsample.CalendarInfo.FIELDS)
					.queue(batch, new JsonBatchCallback<Calendar>() {
						...
						public void onFailure(GoogleJsonError err, HttpHeaders headers) {
							cz.cvut.fit.budikpet.googlecalendarsample.Utils.logAndShowError(activity, CalendarSampleActivity.TAG, err.getMessage());
						}
					});
		}
		batch.execute();
