...
public class MainActivity extends AppCompatActivity {
	...
	private void handleAuthorizationResponse(@NonNull Intent intent) {
		...
		// AuthState stores details of authorization session
	...
	@Override
	// Part of PendingIntent that waits for auth response.
	...
	public static class AuthorizeListener implements Button.OnClickListener {
		...
		public void onClick(View view) {
			...
			// Declares the authorization and token endpoints of the OAuth server we connect to
			AuthorizationServiceConfiguration serviceConfiguration = new AuthorizationServiceConfiguration(
					Uri.parse("https://accounts.google.com/o/oauth2/v2/auth") /* auth endpoint */,
					Uri.parse("https://www.googleapis.com/oauth2/v4/token") /* token endpoint */
			);

			// Create the actual authorization request
	...
	public static class MakeApiCallListener implements Button.OnClickListener {
		...
		public void onClick(View view) {
			...
			// Making API calls inside performActionWithRefreshTokens refreshes access token when needed
			mAuthState.performActionWithFreshTokens(mAuthorizationService, new AuthState.AuthStateAction() {
				@Override
				public void execute(@Nullable String accessToken, @Nullable String idToken, @Nullable AuthorizationException exception) {
					// Fetch and display user account
					new FetchAndDisplayInfo(mMainActivity).execute(accessToken);
				}
			});
	...
	/**
	 * AsyncTask for calling API endpoint.
	 * Gets info about the account - name and image
	 * Shows this info on screen.
	 */
	private static class FetchAndDisplayInfo extends AsyncTask<String, Void, JSONObject> {
		private final MainActivity mMainActivity;

		public FetchAndDisplayInfo(MainActivity mMainActivity) {
